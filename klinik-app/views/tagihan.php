...
try {
    if (isset($db)) {
        // Amankan query di dalam block try agar jika VIEW belum ada, halaman HTML di bawahnya TIDAK IKUT MATI/BLANK
        $query = $db->query("
...
    // Ambil fallback data alternatif (hanya data Belum Lunas) jika VIEW v_dashboard_keuangan_lunas crash/belum dibuat
    try {
...
            WHERE t.status = 'Belum Lunas'
            GROUP BY t.tagihan_id, k.tgl_kunjungan, p.nama, t.diskon
            ORDER BY t.tagihan_id DESC
        ");
        $daftar_tagihan = $query_fallback->fetchAll(PDO::FETCH_ASSOC);
        $error_message = "Catatan Sistem: View 'v_dashboard_keuangan_lunas' belum terpasang di MySQL, menampilkan data tagihan berjalan.";
    } catch(PDOException $ex) {
        $error_message = "Gagal memuat database: " . $ex->getMessage();
    }
}
?>
<!DOCTYPE html>
